...
"""
On va d�finir les coordonn�es des points tels que :
- ce soit des tuples
...
### Calcul de distance : d�finition de notre heuristique

def dist(coordA,coordB):
    return ((coordA[0] - coordB[0])**2 + (coordA[1] - coordB[1])**2)**(1/2)

#C'est �a que l'on comptabilisera comme heuristique dans notre cas (distance la plus courte possible)

### D�finition de la carte
class Point:
    "x, y, cout, heuristique, mur (Booleen), txt"
    #(Cout infini pour que seul les points d�finis puissent �tre pris en compte lors de la r�solution)
...
#On commence avec une grille sans mur ni lieu visit�
for i in range(l):
    for j in range(h):
